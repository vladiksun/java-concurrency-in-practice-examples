 ...
 *
 * Final version, FinalMemorizer takes advantage of the atomic "putIfAbsent"
 * method of ConcurrentMap, closing the window of vulnerability in Memorizer3.
 *
 * FinalMemorizer does not address cache expiration, but this could be accomplished
...
public class FinalMemorizer<A, V> implements Computable<A, V> {
    ...
    public FinalMemorizer(final Computable<A, V> c) {
        this.c = c;
    }
    ...
    public V compute(final A arg) throws InterruptedException {
        while (true) {
            ...
            if (f == null) {
                final Callable<V> eval = () -> FinalMemorizer.this.c.compute(arg);
            ...
            try {
                 ...
                 *
                 * To avoid this, FinalMemorizer removes the Future from cache if it detects that the
